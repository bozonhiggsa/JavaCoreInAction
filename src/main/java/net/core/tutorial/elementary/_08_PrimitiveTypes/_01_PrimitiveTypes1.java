...
/**
 ...
 Тип нового созданного объекта и/или переменной остаются неизменными все их время жизни.
 ...
 Поэтому такой код выдаст ошибку при компиляции:

 Поэтому такой код выдаст ошибку при компиляции:

 byte p = 20;
 byte b = 4;
 byte c = p + b; // результат операции имеет тип int

 Необходимо явное приведение типа от int к byte:

 byte c = (byte) (p + b);

 Но финальные переменные являются константами, а поэтому будут вычислены уже на этапе компиляции. Поэтому такое
 допускается:
 ...
 Компилятор автоматически преобразовывает целые числа в тип, указанный в качестве типа переменной, если эти числа
 умещаются в его диапазон. Поэтому возможны такие конструкции:

 byte p = 20;
 byte p = 'd';
 char ch = 20;
 long number = 100000;
 long number = 10000000000; // ошибка - число выходит за пределы int

...
/**
 ...
 Аналогично, в статическом методе класса нельзя напрямую вызвать нестатический метод этого же класса.
 Ведь нет гарантии, что объект класса существует. Или, если объектов несколько, непонятно, нестатический метод
 какого конкретно объекта надо вызывать. Прежде надо создать конкретный объект класса, а уже у объекта вызвать
 нестатический метод. Также и нестатические методы других классов можно вызвать только создав объекты
 этих классов и обратившись к ним.

 Метод main статический. Поэтому можно считать, что он запускается в момент начала исполнения программы
 по имени класса, в котором он написан. И при этом нет необходимости создавать конкретный объект этого класса.
 В методе main можно обратиться к нестатическим полям и нестатическим методам этого же класса, только если создать
 объект класса, в котором написан метод main.
...
public class _01_StaticAndNonStatic {
    ...
    private int nonStaticField = 100;
    private static int staticField = 300;

    public static void main(String[] args) {

        System.out.println(staticField);
        System.out.println(new _01_StaticAndNonStatic().nonStaticField);
        staticMethod();
        new _01_StaticAndNonStatic().nonStaticMethod();
    }

    public void nonStaticMethod() {
        System.out.println("It's invoke of non static method");
    }

    public static void staticMethod() {
        System.out.println("It's call of static method");
    }
}
